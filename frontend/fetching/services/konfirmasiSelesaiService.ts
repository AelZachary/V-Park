...
export type KonfirmasiSelesaiResponse = {
...
export async function konfirmasiSelesai(bookingID: number): Promise<KonfirmasiSelesaiResponse> {
  const res = await authFetch(`${API_BASE_URL}/api/konfirmasiselesai/booking/${bookingID}`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    throw new Error(extractErrorMessage(payload, `Failed to confirm parking completion (${res.status})`));
  }

  return payload as KonfirmasiSelesaiResponse;
}